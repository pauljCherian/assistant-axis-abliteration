#!/bin/bash
# Abliterated-run variant of hf_sync.sh.
...
#
# Usage:
#   tmux new -s hf-sync-abliterated; bash scripts/hf_sync_abliterated.sh
# Or backgrounded:
#   tmux new-session -d -s hf-sync-abliterated 'bash scripts/hf_sync_abliterated.sh 2>&1 | tee /tmp/hf_sync_abliterated.log'
set -euo pipefail

PROJECT=/scratch/paulc/assistant-axis-abliteration
SCORES=$PROJECT/results/abliterated/scores
STATE=$SCORES/_batch_state.json
STATE_SNAPSHOT=/tmp/_batch_state_snapshot_abliterated.json
RESUME_LOG=$PROJECT/results/abliterated/resume.log
PYTHON=$PROJECT/.venv/bin/python
REPO=pandaman007/assistant-axis-abliteration-vectors
PREFIX=llama-3.1-8b-abliterated
SYNC_INTERVAL_SEC=1800  # 30 min

set -a; source "$PROJECT/.env"; set +a

echo "=== HF sync sidecar (ABLITERATED) starting at $(date) ==="
