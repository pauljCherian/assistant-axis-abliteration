...
| 19 | OpenAI `insufficient_quota` treated as transient rate limit | `openai.RateLimitError` catches both true 429s and billing-exhausted errors; our retry loop hammered a dead API for ~10 hrs | `assistant_axis/judge.py` inspects `e.body["error"]["type"]`; on `insufficient_quota`, sets a module flag, returns None, and skips all future calls — fail-fast |
| 20 | OpenAI Batch API chunks stick at 0 completions | ~30% of batches on Tier 1 never start processing; would deadlock `max_concurrent` slots forever | `3_judge_batch.py::reconcile_batches` cancels batches stuck `in_progress` with 0 completed for >30 min (`STUCK_THRESHOLD_SEC`) |
| 21 | Cancellation stuck in `cancelling` state indefinitely | OpenAI sometimes never finalizes a cancel, so the slot stays "active" forever | After 10 min in `cancelling` with `auto_cancelled=true`, mark batch as `abandoned` (local-only terminal status) and free the slot |
| 22 | Cancelled-batch errors polluting `permanent_failures` | When a batch cancels before processing, error file lists all 1000 prompts as `batch_cancelled`; old code added them to `permanent_failures`, blocking retry forever → **silent data loss** | Only add errors to `permanent_failures` when `info["status"] == "completed"`. Cancelled/expired/abandoned/failed batches must NOT pollute |
| 23 | Enumerator re-submitting in-flight prompts on restart | On kill+restart, the enumerator didn't know about already-submitted-but-not-merged batches, so it re-chunked their prompts into new batches (duplicate API calls + wasted $) | Store `custom_ids` list per-batch in state file; `backfill_custom_ids` reconstructs from `files.content(input_file_id)` for batches that predate this fix. Enumerator excludes `in_flight` set |
| 24 | Judge exits prematurely when abandoned batches free unscored prompts | `chunks` was computed once before main loop; prompts returned to unscored pool mid-run weren't picked up until another process restart | Wrap enumerate + main loop in an outer `while True` — after inner loop drains, re-enumerate; break only when both `chunks` empty AND `get_active_ids()` empty |
| 25 | Corrupt score JSON silently eaten | `3_judge.py` line 259-264: `try/except` around `json.load()` set `existing_scores = {}` on parse failure, silently discarding all prior scores on the next save | `load_existing_scores` raises `RuntimeError` on corrupt file — refuses to continue. User must inspect manually. |
| 26 | Partial writes can corrupt `{role}.json` on crash | A crash during `json.dump(f)` leaves a truncated file | Atomic write: `tempfile.mkstemp` + `os.fsync` + `os.replace` in `atomic_write_json` |
| 27 | Accumulating OpenAI input files hit account quota | Each batch uploads a ~500KB JSONL file; 600+ batches over both pipelines = 300MB, some tier limits hit | `client.files.delete(input_file_id)` after successful merge; output/error files retained for audit |
| 28 | HF backup only at pipeline end → single point of failure | 15-20hr run with one terminal upload = if lisplab1 crashes at hr 10, state file is gone, all in-flight OpenAI work disconnected from role/key mapping | `scripts/hf_sync.sh` sidecar uploads `scores/` + `_batch_state.json` every 30 min to `llama-3.1-8b-instruct/_state/` on HF — disaster-recoverable |
| 29 | Duplicate simultaneous pipelines could corrupt state | If two copies of `3_judge_batch.py` run against the same scores dir, they'll race on state file writes | `fcntl.flock(LOCK_EX \| LOCK_NB)` on `_batch.lock` — second instance exits immediately with clear message |
